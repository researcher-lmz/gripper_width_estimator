"""
夹爪宽度估计 —— MCAP 输入/输出版本（左右双相机，独立 ID 方案，无 ROI）

= mcap_gripper_width.py 的 MCAP I/O + 双相机 + 增广输出
+ quick_test_unique_id.py 的检测方案：每个夹爪贴【不同】tag，靠唯一 ID 区分，不用 ROI。
  左夹爪两指 = (0,1)，右夹爪两指 = (2,3)。一个相机即使看到另一夹爪，ID 不冲突。

输入：MCAP /camera/<cam>/compressed 解码 JPEG
输出：每路相机一个 /umi/gripper_width/<cam> topic；COPY_ORIGINAL=True 时含原始全部消息
运行：/Users/limingzhe/anaconda3/envs/x/bin/python3 mcap_gripper_width_unique_id.py
"""
...
# 每个夹爪贴独立 ID（不需要 ROI）：左夹爪两指 (0,1)，右夹爪两指 (2,3)
TAG_IDS = {
    "wrist_left":  (0, 1),
    "wrist_right": (2, 3),
}

# 每路相机的标定来源（mp4 或 mcap）。各相机像素距离不同，理应各自标定。
CALIB_SOURCES = {
    "wrist_left":  "/Users/limingzhe/all_github_code/gripper_width_estimator/标定片段_left.mp4",
    "wrist_right": "/Users/limingzhe/all_github_code/gripper_width_estimator/标定片段_right.mp4",
}
...
def load_calib_frames(path):
    """标定片段：mp4 直接读；mcap 取第一个 camera topic。"""
    ...
    return [fr for _, fr in read_camera_frames(path, topic)] if topic else []
...
def process_camera(cam, task_frames_ns):
    """对一路相机：标定 + 逐帧估计 + 后处理 → 返回 width_records。
    用该夹爪自己的独立 tag ID，不用 ROI。"""
    print(f"\n===== 处理相机 {cam} =====")
    timestamps = [ns for ns, _ in task_frames_ns]
    frames = [fr for _, fr in task_frames_ns]

    left_tag, right_tag = TAG_IDS[cam]
    print(f"  tag IDs = {left_tag},{right_tag}（独立 ID，无 ROI）")

    est = GripperWidthEstimator(
        closed_width_mm=CLOSED_DIST_MM, open_width_mm=OPEN_DIST_MM,
        left_tag_id=left_tag, right_tag_id=right_tag,
        use_fisheye=False, detector_backend="apriltag", apriltag_family="tag36h11",
        detection_roi=None,            # 独立 ID 区分，不用 ROI
        max_std_d=MAX_STD_D, min_valid_frames=12,
    )

    calib_src = CALIB_SOURCES.get(cam)
    if not calib_src or not os.path.exists(calib_src):
        print(f"  [WARN] {cam} 标定来源不存在({calib_src})，跳过")
        return None
...
def main():
    ...
    print(f"处理相机: {CAMERAS}（独立 ID 方案，无 ROI）")

    width_topics = {}
    for cam in CAMERAS:
        task = read_camera_frames(MCAP_IN, f"/camera/{cam}/compressed")
        if not task:
            print(f"\n[WARN] {cam}: 任务 MCAP 里没有该相机帧，跳过")
            continue
        records = process_camera(cam, task)
        if records:
    ...
    out_path = (f"outputs/{stem}_with_width_{ts}.mcap" if COPY_ORIGINAL
                else f"outputs/gripper_width_{stem}_{ts}.mcap")
