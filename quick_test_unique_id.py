"""
夹爪宽度估计 —— 独立 ID 方案（无 ROI）

与 quick_test.py 的区别：
  - 左右夹爪贴【不同】的 tag：左夹爪两指 = (0,1)，右夹爪两指 = (2,3)
  - tag ID 唯一 → 一个相机即使同时看到另一只夹爪，ID 也不冲突
    → 不需要 ROI 区分，DETECTION_ROI 移除
  - 处理哪个相机就选哪个夹爪的 ID（GRIPPER 配置）

用法：
  /Users/limingzhe/anaconda3/envs/x/bin/python3 quick_test_unique_id.py
"""
...
# 每个夹爪贴独立 ID：左夹爪两指 (0,1)，右夹爪两指 (2,3)。
# 处理哪路相机就把 GRIPPER 设成对应夹爪。ID 唯一 → 不需要 ROI。
GRIPPER = "left"                   # "left" 或 "right"
TAG_IDS = {"left": (0, 1), "right": (2, 3)}
LEFT_TAG_ID, RIGHT_TAG_ID = TAG_IDS[GRIPPER]   # 一只夹爪的两个手指 tag（只算距离，左右无所谓）

VIDEO_PATH = "/Users/limingzhe/all_github_code/gripper_width_estimator/dataset_06172023/wrist_left_camera.mp4"
# 标定专用片段（闭合保持 + 张开到机械最大保持）。None = 从 VIDEO_PATH 自标定。
CALIB_VIDEO = "/Users/limingzhe/all_github_code/gripper_width_estimator/标定片段.mp4"
FPS = 60
OUTPUT_VIDEO = "outputs/visualized_result.mp4"

# 鱼眼去畸变：实验证明没必要（精度相当甚至略好），关掉更简单
USE_FISHEYE = False
...
def main():
    print(f"夹爪 = {GRIPPER}，tag IDs = {LEFT_TAG_ID},{RIGHT_TAG_ID}（独立 ID，无 ROI）")
    ...
    # 2. 初始化估计器（detection_roi=None：靠唯一 ID 区分，不用 ROI）
    estimator = GripperWidthEstimator(
        closed_width_mm=CLOSED_DIST_MM,
        open_width_mm=OPEN_DIST_MM,
        left_tag_id=LEFT_TAG_ID,
        right_tag_id=RIGHT_TAG_ID,
        use_fisheye=USE_FISHEYE,
        camera_matrix=K,
        distortion_coeffs=D,
        detector_backend="apriltag",
    ...
    if not valid:
        print("\n标定失败：检查 tag 是否检测到、夹爪标定时是否完全闭合/张开保持。")
        print("继续进行估计（用不完美的标定值）...")
    ...
    for i, frame in enumerate(frames):
        ...
        # 用原始检测画框：只画本夹爪的两个 tag（同 ID 取 margin 最高）
        ...
        cv2.putText(vis, f"Frame: {i}/{len(frames)}  gripper={GRIPPER} tags={LEFT_TAG_ID},{RIGHT_TAG_ID}",
        ...
        cv2.imshow("Gripper Width (unique-ID, no ROI)", vis)
